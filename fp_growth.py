...
def find_frequent_itemsets(transactions, minimum_support, include_support=False):
	...
	#master.inspect()
	#print [x for x in  master.prefix_paths('d')]
	#conditional_tree_from_paths(master.prefix_paths('d'),
	#	minimum_support).inspect()
...
class FPNode(object):
	...
	def merge_and_remove(self, other_node):
		assert other_node.item == self.item
		
		self._count += other_node.count
		
		for child in other_node.children:
			if child._item in self._children:
				self._children[child._item].merge_and_remove(child)
			else:
				self.add(child)
		
		other_node.parent = None
		self._tree._removed(other_node)
			...
			if self._children[child.item] is child:
				...
				for sub_child in child.children:
					try:
						...
						self._children[sub_child.item].merge_and_remove(sub_child)
						#self._children[sub_child.item]._count += sub_child.count
						#sub_child.parent = None # it's an orphan now
						
						# remove the merged child from route
						#self._tree._removed(sub_child)
					except KeyError:
